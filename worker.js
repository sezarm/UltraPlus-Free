...
function renderConfigs(lang, host) {
  return baseLayout(lang, t(lang,"configs"), `<h2>${t(lang,"configs")}</h2><div class="note">/sub/<uuid> — base64 | raw | clash<br>VLESS WS 0.6.5: path must match Settings. TCP only; max 48 concurrent WS.</div><p>Base: https://${host}/sub/<uuid></p>`);
}
function renderSettings(lang, settings) {
  settings = settings || { path: "/", remark: "UltraPlus", sni: "" };
  return baseLayout(lang, t(lang,"settings"), `<h2>${t(lang,"settings")}</h2><div class="warn">${t(lang,"passWarning")}<br>${t(lang,"defaultPass")}</div><form method="POST" action="/admin/settings/save" style="padding:1rem;background:var(--c);border-radius:.75rem"><div>WS Path (must match client)<br><input name="path" value="${(settings.path||"/").replace(/"/g,"")}"></div><div>Remark<br><input name="remark" value="${(settings.remark||"UltraPlus").replace(/"/g,"")}"></div><div>SNI<br><input name="sni" value="${(settings.sni||"").replace(/"/g,"")}"></div><input type="hidden" name="lang" value="${lang}"><button class="btn" type="submit">Save</button><p style="font-size:.8rem;opacity:.7">Needs KV. Path used by proxy core.</p></form><div class="note">Bot: /status /stats /users /add /toggle /del /link /help</div>`);
}
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/health") {
      const users = await loadUsers(env);
      return Response.json({ status: "ok", project: "UltraPlus-Free", version: "0.6.5", phase: "7.5", users: users.length, active: users.filter(isUserValid).length, kv: !!env.ULTRA_KV, telegram: !!env.TELEGRAM_BOT_TOKEN, proxy: true, pathMatch: true, hardened: true, queueCap: 64, stats: proxyStats, maxActive: MAX_ACTIVE });
    }

    return new Response("UltraPlus-Free v0.6.5 – /admin or /wizard", { headers: { "Content-Type": "text/plain;charset=utf-8" } });
  },
};
