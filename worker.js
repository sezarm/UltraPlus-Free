...
const translations = {
  en: {
    ...
    phase: "v0.4", uuid: "UUID", actions: "Actions",
  ...
  fa: {
    ...
    phase: "نسخه ۰.۴", uuid: "UUID", actions: "عملیات",
  ...
  zh: {
    ...
    phase: "v0.4", uuid: "UUID", actions: "操作",
...
function renderUsers(lang, host, users) {
  ...
  if (users.length === 0) {
    ...
    for (const u of users) {
      ...
      rows += `<tr><td>${u.name}</td><td style="font-size:.75rem">${u.uuid.slice(0,8)}...</td><td>${u.enable?t(lang,"enable"):t(lang,"disable")}</td><td>${exp}</td><td>${tr}</td><td><a class="btn" href="${sub}" target="_blank">${t(lang,"subLink")}</a><form method="POST" action="/admin/users/delete" style="display:inline"><input type="hidden" name="id" value="${u.id}"><input type="hidden" name="lang" value="${lang}"><button class="btn btn-d" type="submit">${t(lang,"delete")}</button></form></td></tr>`;
    }
  }
...
function renderSettings(lang) {
  const body = `<h2>${t(lang,"settings")}</h2><div class="warn"><strong>${t(lang,"important")}</strong><br>${t(lang,"passWarning")}<br><br>${t(lang,"defaultPass")}</div><div class="note">${t(lang,"kvNote")}<br><br>${t(lang,"botNote")}<br>Webhook: <code>https://api.telegram.org/bot&lt;TOKEN&gt;/setWebhook?url=https://YOUR-WORKER/telegram</code></div>`;
  ...
  async fetch(request, env) {
    ...
    if (path === "/telegram" && request.method === "POST") {
      try {
        ...
        if (text === "/start") {
          await sendTelegram(env, chatId, isAdmin ? "✅ UltraPlus-Free Admin\n/status /users /help" : "This bot is private.");
        } else if (!isAdmin) {
          ...
          const users = await loadUsers(env);
          const list = users.length ? users.map(function(u){return "• " + u.name + " | " + (u.enable?"ON":"OFF") + " | " + u.uuid.slice(0,8);}).join("\n") : "No users";
