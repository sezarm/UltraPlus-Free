/**
 * UltraPlus-Free v0.4.1 - Single File Worker
...
const translations = {
  en: {
    ...
    phase: "v0.4.1", uuid: "UUID", actions: "Actions",
  ...
  fa: {
    ...
    phase: "نسخه ۰.۴.۱", uuid: "UUID", actions: "عملیات",
  ...
  zh: {
    ...
    phase: "v0.4.1", uuid: "UUID", actions: "操作",
...
function renderUsers(lang, host, users) {
  ...
  if (users.length === 0) {
    ...
    for (const u of users) {
      ...
      const statusBadge = u.enable
        ? `<span class="badge">${t(lang,"enable")}</span>`
        : `<span class="badge badge-off">${t(lang,"disable")}</span>`;
      const toggleLabel = u.enable ? t(lang,"disable") : t(lang,"enable");
      const toggleClass = u.enable ? "btn-w" : "btn-s";
      rows += `<tr>
        <td>${u.name}<div class="sub-box">${sub}</div></td>
        ...
        <td>
          ...
          <form method="POST" action="/admin/users/toggle" style="display:inline">
            ...
            <button class="btn ${toggleClass}" type="submit">${toggleLabel}</button>
          </form>
  ...
  const body = `<h2>${t(lang,"users")}</h2>
    ...
    <table>
      <thead><tr>
        <th>${t(lang,"name")} / Link</th><th>${t(lang,"uuid")}</th><th>${t(lang,"status")}</th>
...
export default {
  async fetch(request, env) {
    ...
    if (path.startsWith("/admin")) {
      ...
      if (path === "/admin/users/toggle" && request.method === "POST") {
        ...
        users = users.map(function(u) {
          if (u.id === id) {
            return Object.assign({}, u, { enable: !u.enable });
          }
          return u;
        });
        await saveUsers(env, users);
    ...
    return new Response("UltraPlus-Free v0.4.1 – /admin or /wizard", {
